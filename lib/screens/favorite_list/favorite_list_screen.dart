...
class FavoriteListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        if (controller.isLoading.value && controller.favoriteList.isEmpty) {
          return _buildShimmerGrid();
        }

        if (controller.favoriteList.isEmpty && !controller.isLoading.value) {
          return _buildEmptyState();
        }

        return _buildFavoriteGrid(controller);
      }),
    );
  }
  ...
  Widget _buildFavoriteGrid(FavoriteListController controller) {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        if (index >= controller.favoriteList.length) {
          // Show shimmer for loading more
          return _buildShimmerCard();
        }
        final movie = controller.favoriteList[index];
        return _buildFavoriteItem(movie, controller);
      },
    );
  }
  ...
  Widget _buildFavoriteItem(
    MovieModel movie,
    FavoriteListController controller,
  ) {
    return InkWell(

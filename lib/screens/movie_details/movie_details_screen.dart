...
class MovieDetailsScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        ...
        return CustomScrollView(
          slivers: [
            _buildAppBar(movie, controller),
            SliverToBoxAdapter(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _buildMovieHeader(movie, controller),
                  SizedBox(height: 12.h),
                  _buildActionButtons(controller),
                  SizedBox(height: 12.h),
                  _buildGenreInfo(movie),
                  SizedBox(height: 12.h),
                  _buildTagsSection(movie),
                  SizedBox(height: 12.h),
  ...
  Widget _buildAppBar(MovieModel movie, MovieDetailsController controller) {
    // Get list of images - use images array, fallback to poster if no images
    final List<String> imageUrls = [];
    if (movie.images.isNotEmpty) {
      imageUrls.addAll(
          movie.images.map((img) => img.url).where((url) => url.isNotEmpty));
    }
    // If no images but has poster, use poster as single image
    if (imageUrls.isEmpty && movie.poster != null && movie.poster!.isNotEmpty) {
  ...
  Widget _buildMovieHeader(
      MovieModel movie, MovieDetailsController controller) {
    return Padding(
  ...
  Widget _buildActionButtons(MovieDetailsController controller) {
    return Obx(() {
      final movie = controller.movieDetail.value;
      final isPremium = movie?.type == ContentType.subscription;
          ...
          children: [
            ...
            Expanded(
              child: GetBuilder<MovieDetailsController>(
                builder: (controller) {
                  final isBuyable = movie?.type == ContentType.buy;
                  return CustomButton(
                    text: (movie?.isPurchased == true)
                        ? AppStrings.play.tr
                        : isBuyable
                            ? AppStrings.buy.tr
                            : showSubscribe
                                ...
                                : AppStrings.play.tr,
                    onPressed: (movie?.isPurchased == true)
  ...
  Widget _buildGenreInfo(MovieModel movie) {
    if (movie.category.isEmpty) return SizedBox.shrink();
  ...
  Widget _buildTagsSection(MovieModel movie) {
    if (movie.tags.isEmpty) return SizedBox.shrink();
  ...
  Widget _buildCastSection(MovieModel movie) {
    if (movie.cast.isEmpty) return SizedBox.shrink();

    // Get all cast profile images for full-screen viewer
    final castImages = movie.cast
